...
(function($){
	...
	rCalendar.prototype = {
		...
		_renderingWidgetDays: function() {
			...
			var temp = this.getEventsCurrentDateX(currentDate);
			//this.getBooking(temp);
			//alert(temp[0]['html']);
			var tbody = $("<div class='r-calendar-body'></div>");
			for(var i = 0; i < 24; i++) {
				...
				tbody.append($("<div class='r-calendar-daytime'>"
					...
					+ "<div class='r-calendar-daytime-grid r-calendar-daytime-grid-90 r-calendar-daytime-active' data-date='" + currentDate + "' data-time='" + currentTime + "'>" + this.getBooking(temp, currentTime.substr(0,2)) + "</div>"
					+ "</div>"));
			}
			rCalendarWidget.append(tbody);
			this.$el.find('.r-calendar-widget').remove();
...
//alert(startDate + " " + endDate + " " + dd);
					...
					if((dd.getFullYear() == startDate.getFullYear()) && (dd.getMonth() == startDate.getMonth()) && (dd.getDate() == startDate.getDate())) {
						...
						countEvents += 1;
						htmlBadgeEvents = "<span class='r-calendar-badge' title='" + this.loc.titleEvents + "' style='height:" + (50*diff) + "px;'>" + this.loc.events + ":&nbsp;" + String(countEvents) + "</span>";
				...
				/*if(Number(countEvents) > 0)
					htmlBadgeEvents = "<span class='r-calendar-badge' title='" + this.loc.titleEvents + "' style='" + (50*diff.hours) + "px;'>" + this.loc.events + ":&nbsp;" + String(countEvents) + "</span>";
				*/
			} catch(e) {
				htmlBadgeEvents = "";
			}
